...
contract EchidnaTemplate {
    ...
    function add_test_associative(int128 x, int128 y, int128 z) public {
        int128 x_y = add(x,y);
        int128 x_y_z= add(x_y, z); 

        int128 y_z = add(y,z);
        int128 y_z_x = add(y_z, x);
        assert(x_y_z == y_z_x);
    }
    ...
    function add_sub_inverse_operations(int128 x, int128 y) public {
        emit Debug(x,y);
        int128 x_y = add(x,y);
        int128 x_y_y = sub(x_y, y);
        emit Debug(x_y, x_y_y);
        assert(x_y_y == x);
    }
    ...
    function div_test_not_commutative(int128 x, int128 y) public {
        // Pre condition (barriers to entry)
        require(abs(x) != abs(y)); // bounding input.
        // optimisation don't waste CPU cycles that could be used testing.
        if (abs(x) == abs(y)) {
            y = x + 1;
        }

        // Action: what are we testing?
        int128 x_y = div(x, y);
        int128 y_x = div(y,x);

        // Post condition: What are the truths?
        if (abs(x)==abs(y)) {
            assert(x_y == y_x);
        } else {
            assert(x_y != y_x);
